...
public class ServerHandler implements HttpHandler {
    ...
    private Response doPost(HttpExchange httpExchange) {
        try {
            ...
            logger.info(String.format(" sended mes : %s", message));
    ...
    private Response doPut(HttpExchange httpExchange) {
        ...
        try {
            Message message = MessageHelper.getClientMessageById(httpExchange.getRequestBody());
    ...
    private Response doDelete(HttpExchange httpExchange) {
        String qry = httpExchange.getRequestURI().getQuery();
        if (qry == null) {
            return Response.badRequest("Absent query in request");
        }else {
            Map<String, String> qm = queryToMap2(qry); // лямбда - это классно =)
            String id = qm.get(Constants.REQUEST_PARAM_MESSAGE_ID);
            if (id == null || id.length() == 0) {
                return Response.badRequest("Wrong id ");
            }else {
                try {
